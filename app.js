  ...
  rows.innerHTML =
    filtered
      .map(
        ...
        <tr class="${x.handover === 'COPY' ? 'copied-handover-row ' : ''}${x.status === 'Open' ? 'row-status-open' : x.status === 'Closed' ? 'row-status-closed' : x.status === 'On Hold' ? 'row-status-hold' : ''}" data-row-id="${esc(x.id)}">
